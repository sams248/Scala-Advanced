...
class Cons[+A](h: A, t: => MyStream[A]) extends MyStream[A] {
  ...
  override lazy val tail: MyStream[A] = t // combining call by name and lazy evaluation = call by need
  ...
  /*
  val s = new Cons(1, tail)
  val mapped = s.map(_ + 1) = new Cons(2, s.tail.map(_ + 1)
 */
  ...
  def filter(predicate: A => Boolean): MyStream[A] = {
    if (predicate(h)) new Cons(head, tail.filter(predicate))
