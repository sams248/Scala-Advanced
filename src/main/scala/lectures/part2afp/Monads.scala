...
object Monads extends App {
  ...
  /*
    ...
    1) Implement a Lazy[T] monad = computation which will only be executed when it's needed.
       Implement unit/apply & flatMap

    2) Monads = unit + flatMap
       Monads = unit + map + flatten // alternative definition

       Monad[T] {
        def flatMap[B](f: T => Monad[B]): Monad[B] = ... (implemented/given)
        def map[B](f: T => B): Monad[B] = ??? how to implement this?
  ...
  val lazyInstance = Lazy {
    println("I don't feel like doing anything today!")
    42
  }
